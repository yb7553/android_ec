package com.flj.latte.dialog.pickerview.lib;
...
import com.flj.latte.dialog.pickerview.adapter.WheelAdapter;
...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textColorOut = getResources().getColor(R.color.common_gray_5);
        textColorCenter = getResources().getColor(R.color.common_black_1);
        dividerColor = getResources().getColor(R.color.common_gray_7);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                String paintText = getContentText(visibles[counter]);
                String contentText = paintText;
                if(contentText != null && contentText.length() >0) {
                    float textWidth = paintOuterText.measureText(contentText);
                    if ( textWidth> measuredWidth) {
                        int subIndex = paintOuterText.breakText(contentText, 0, contentText.length(), true, measuredWidth, null);
                        contentText = paintText.substring(0, subIndex-3)+"...";
                    }
                }
                //计算开始绘制的位置
