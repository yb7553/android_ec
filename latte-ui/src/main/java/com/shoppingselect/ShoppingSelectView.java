...
public class ShoppingSelectView extends LinearLayout {
    ...
    private int defalutcolor =ButtonColor.defalut;
    private int selectcolor = ButtonColor.select;
    ...
    public void getView() {
        ...
        for (BigClassification attr : list) {
            ...
            for (int i = 0; i < attr.getList().size(); i++) {
                ...
                button.setTextColor(defalutcolor);
                //默认选中第一个
                if (i == 0) {
                    button.setTextColor(selectcolor);
                    attr.getList().get(i).setSelect(true);
